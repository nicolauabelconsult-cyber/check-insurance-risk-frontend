...
// O backend atual valida period como: 7d | 30d | 90d | 12m (ver Dashboard.tsx)
...
export default function ExecutiveDashboard() {
  ...
  const qs = useMemo(() => `period=${encodeURIComponent(period)}`, [period]);
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const t = await apiFetch(`/dashboard/trends?${qs}&granularity=${period === "12m" ? "week" : "day"}`);
