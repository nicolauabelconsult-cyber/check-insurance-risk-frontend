...
export default function RiskCreate() {
  ...
  const [entityId, setEntityId] = useState(""); // só admins
  ...
  const search = async () => {
    ...
    const payload: any = { name: q, nationality: nationality.trim() || undefined };
    if (isAdmin) payload.entity_id = entityId.trim();

    const res = await risksApi.search(payload);
  ...
  return (
    <>
      ...
      <div className="toolbar" style={{ justifyContent: "flex-start", gap: 12 }}>
        {isAdmin && (
          <div style={{ width: 360 }}>
            <label>Entity ID (apenas Admin)</label>
            <input className="input" style={{ width: "100%" }} value={entityId} onChange={(e) => setEntityId(e.target.value)} />
          </div>
        )}
          ...
          {picked && (
            <div className="card" style={{ padding: 14, marginTop: 14 }}>
              ...
              <div className="toolbar" style={{ justifyContent: "flex-start", gap: 12 }}>
                ...
                <button className="btn primary" onClick={confirm}>Gerar análise</button>
              </div>
            </div>
          )}
        </div>
